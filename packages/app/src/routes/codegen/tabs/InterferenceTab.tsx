        ...
        const nodes: ElkGraphNode[] = (finalFn?.graph.nodes ?? []).map((n) => ({
          ...
          reactNode: (
            <span className="flex items-center gap-1.5">
              ...
              <span className="text-3xs text-ink-faint">{n.kind}</span>
              <span className="rounded-full bg-raised px-1.5 text-3xs text-ink-muted tabular-nums">
        ...
        return (
          <>
            ...
            <Panel
              ...
              // The key sits in the head of the artifact it explains.
              actions={
                <span className="flex flex-wrap items-center gap-x-4">
                  <span className="section-meta">
                    {finalFn
                      ? `${finalFn.graph.nodes.length} nodes · ${added.size}/${finalFn.graph.edges.length} edges`
                      : ''}
                  </span>
                  <Legend
                    items={[
            ...
            {/* Per-step justification, not standing prose: it changes with the
                cursor and names the instruction that forced this decision. */}
            <Panel title="Justification">
              {currentEdge !== null ? (
                <div className="flex flex-col gap-2.5 text-sm">
                  <p className="flex flex-wrap items-center gap-2">
                    <Link2 aria-hidden className="size-4 text-accent" />
                    <span className="font-mono type-code text-ink">
                      {currentEdge.a} — {currentEdge.b}
                    </span>
                    <Tag tone="accent">instruction {currentEdge.atIndex}</Tag>
                  </p>
                  <pre className="framed artifact-scroll bg-code px-3 py-2 font-mono type-code text-ink">
                    {currentEdge.atIndex}: {justifyingInstr ? formatVInstr(justifyingInstr) : '—'}
                  </pre>
                  <p className="text-ink-muted">
                    Defines <span className="font-mono text-ink">{currentEdge.def}</span> while{' '}
                ...
                <div className="flex flex-col gap-2 text-sm">
                  ...
                  <p className="text-ink-muted">
                    A copy at instruction {currentException.atIndex}: source and target may share a
                    register.
                  </p>
                </div>
              ) : currentForbid !== null ? (
                <div className="flex flex-col gap-2 text-sm">
                  ...
                  <p className="text-ink-muted">
                    {currentForbid.reg} is live across instruction {currentForbid.atIndex}.
                  </p>
                </div>
              ) : (
                <p className="prose-note">Step forward.</p>
              )}
            </Panel>

            <div className="cg-row mt-8 grid gap-8 md:grid-cols-2">
              <Panel title="Precoloring">
                {Object.keys(liveFn?.forbidden ?? {}).length === 0 ? (
                  <p className="prose-note">None yet.</p>
                ) : (
                  <ul className="flex flex-col gap-1 font-mono type-code">
                    {Object.entries(liveFn?.forbidden ?? {}).map(([node, regs]) => (
                      <li key={node} className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="text-2xs text-ink-faint">must avoid</span>
                        {regs.map((r) => (
                          <Tag key={r} tone="warn">
                            {r}
                          </Tag>
              ...
              <Panel title="Move-related pairs">
                {(liveFn?.moves.length ?? 0) === 0 ? (
                  <p className="prose-note">None yet.</p>
                ) : (
                  <ul className="flex flex-wrap gap-x-4 gap-y-1 font-mono type-code text-ink-muted">
                    {(liveFn?.moves ?? []).map((m) => (
                      <li key={`${m.a}-${m.b}`}>
                        {m.a} ↔ {m.b}
                      </li>
